...
import { authenticateUser } from './permissions/index.js';
...
// express router
app.get('/order', authenticateUser, catchAsync(controllers.getOrders));
app.patch('/order', authenticateUser, catchAsync(controllers.updateOrders));
app.post('/order', authenticateUser, catchAsync(controllers.createOrders));
...
mongoose.connect(
  ...
  (error) => {
    if (error) {
      ...
      console.log(error);
      app.listen(PORT, () => console.log(`Server is running on ${PORT} WITHOUT MONGO`));
    } else {
